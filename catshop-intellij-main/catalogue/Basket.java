...
public class Basket extends ArrayList<Product> implements Serializable
{
   ...
   * Product is appended from the existing products
   * in the basket.
   * @param pr A product to be removed from the basket
   * @return true if successfully removed the product
   */
  public boolean remove( Product pr )
  {                              
	//remove product with the same name like in betterBasket, so we can remove any item
	for ( Product oldpr: this ) {
		if (oldpr.getProductNum().equals(((Product) pr).getProductNum())){
			return super.remove( oldpr );     // Call remove in ArrayList
		}
	}
	return false;
  }
