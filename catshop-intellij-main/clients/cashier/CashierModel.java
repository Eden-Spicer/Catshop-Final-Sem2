...
public class CashierModel extends Observable {
	 ...
	 * Remove the product
	 */
	public void removeProduct(String productN) {
		pn = productN.trim();
		String theAction = "";
		int amount = 1; // & quantity
		try {
			if (theBasket != null & theProduct != null) { // Check Basket exists
				if (theBasket.remove(theProduct)) { // remove product	
					Product pr = theStock.getDetails(pn);
					theProduct = pr;
					theProduct.setQuantity(amount);
					theStock.addStock(pn, amount);
					theAction = "Removed " + theProduct.getDescription(); // fix the stock
				} else {
					theAction = "You cannot remove this product";
				}

			} else {
				theAction = "The basket is empty";
			}
		} catch ( StockException e) {// handle exception error
			DEBUG.error("%s\n%s", "CashierModel.doBuy", e.getMessage());
			theAction = e.getMessage();
		...
		setChanged();
		notifyObservers(theAction); // update model
